%function mainGUI(varargin)
function mainFig = mainGUI(fileName)

        fileName= 'D:\myMATLAB\LaserDAQ\CFG\myConfig.ini'; % Remove when ready to deploy.
...
%%   Create a container figure
        mainFig = figure(...
            ...
            'DockControls', 'off',...
            'Color', White);
        set(mainFig,'deletefcn',{@GUI_close, mainFig}); %{@GUI_close, mainFig}
        set(mainFig, 'units', 'normalized');
        setappdata(mainFig, 'BGColor', BGColor);
        set(mainFig, 'SizeChangedFcn', @GUI_resize);

     % Create application data for mainFig (move to main or to init?)
     ...
     % Read coordinates file
        ...
        cameraY = str2double(configData.camera.cameray);%1392
    ...
    % Create indicators
       %TEMPERATURE INDICATOR FOR INFRARED SENSOR
        tIndX= 0.48;
       ...
       %TEMPERATURE IR LABEL
        ...
        %TEMPERATURE INDICATOR FOR NTC SENSOR
...
%%  CREATE TAB GROUP
    % Prevent an annoying warning msg
        ...
        mainTabGroup = uitabgroup(...
            'Parent', mainFig,...
            'Units', 'normalized',...
            'Position', [0.01 0.01 0.48 0.85],...%0.98
            'TabLocation','top');
           
%% Create individual tabs
        tab_1 = uitab('parent', mainTabGroup ,'title','SCAN');
        tab_2 = uitab('parent', mainTabGroup ,'title','MANUAL'); %#ok<NASGU>
...
%% Close button  (visible in all tabs)
        closeBtn= uicontrol(...
            ...
            'String','CLOSE',...
            'Units', 'normalized',...
            'Position', [0.94 0.01 1.1*buttonW 1.1*buttonH], ...'Position', [MaxWindowX-1.5*myButtonW 0.5*myButtonH myButtonW myButtonH], 
            'FontSize', 10, ...
            'Callback', {@closeBtn_call});
            %'Callback', {@GUI_close, mainFig});
            %delete(gcbf);
...
%% Preview area in main figure
        ...
        %blankimage = zeros(cameraY/2, cameraX/2, 3, 'uint8');
        ...
        prevHaxes = axes(...
            ...
            %'Position', [810 100 cameraX/2 cameraY/2]); %
            %'Position', [820 230 cameraY/2 cameraX/2]); % Define size half of image resolution
        set(prevHaxes, 'Units', 'Normalized'); % This is to adjust window when resizing
        ...
        setappdata(previewImage,'UpdatePreviewWindowFcn',@GUI_editPreview); % ROTATE IMAGE TO MAKE IT STRAIGHT IN PREVIEW
        ...
        % CHECKBOX FOR PREVIEW NORMALIZATION
        ...
        % VALUE OF PREVIEW NORMALIZATION
        prevValLbl= uicontrol('Style','text',...
                'Units', 'normalized',...
                'Parent', mainFig,...
        ...
        %histoPrevLine = imline(histoHaxes, [0.5 0.025; 0.5 0.031 ]);
        %histoPrevLine= line([255,255], [0,50], 'Parent', histoHaxes, 'color', 'black');
        %setappdata(gcf, 'histoPrevLine', histoPrevLine);
...
%%   TAB 1 "AUTO" controls
      ...
      % ====ADJUST====
        uicontrol('Parent', tab_1, ... %%ADJUST (allows to reset the position after manual adjustment)
            'Units', 'normalized', ...
            ...
            'FontSize', 10, ...
            'TooltipString', sprintf('this is line 1 of my tooltip\nand this is line 2') );
...
% If user closes mainFig, close new one as well.
    %set(mainFigHandle,'deletefcn',{@fig_delet, configGUI, mainFigHandle});
...
function [] = fig_delet(varargin)
    %% Executes when user closes gui_passdata.
        try
            delete(varargin{3})
            GUI_close(varargin{4}); % Just in case.
        catch
            % Do nothing.
        end
